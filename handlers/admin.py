...
#@dp.message_handler(commands=['moderator'], is_chat_admin=True)-используем для написания бота в одном файле
...
async def load_description(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Туперь укажи цену')
...
#Удаление из меню для администратора
